...
#### Windows 自动启动、失效重建与 ntfy 新链接

`start_remote_codex.ps1` 是 Windows 专用便捷脚本。它会启动本地服务和 Quick Tunnel；如果 Cloudflare 已回收临时通道，脚本会重建通道。配置 `watch.env` 中的 `NTFY_NOTIFY_TOPIC` 后，地址变化时会向 ntfy 发送一条 `Codex Remote - NEW LINK` 通知，正文包含完整地址，并带有 `OPEN CODEX` 按钮。

先确认已安装 `cloudflared`，并已按[通知与审批](./docs/NOTIFICATIONS.md)配置 ntfy，然后执行：

```powershell
.\start_remote_codex.ps1
```

Quick Tunnel 可能在进程仍存活时被 Cloudflare 回收。需要长期运行时，可安装每 5 分钟执行一次的当前用户计划任务；它在使用电池时也会运行，并且只在地址变化时发送新通知：

```powershell
.\start_remote_codex.ps1 -InstallWatchdog
```

停用远程访问时，先删除巡检任务，避免它重新启动通道，然后再结束 cloudflared 和 PocketCodex 进程：
...
- `start_remote_codex.ps1` 是 Windows + cloudflared + ntfy 便捷脚本，可安装或删除 `RemoteCodexWatchdog`；通用安装建议先使用本 README 中的手动命令。
