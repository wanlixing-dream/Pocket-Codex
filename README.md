...
- 每台电脑各自启动 Cloudflare Quick Tunnel 或 Tailscale Serve，并得到不同的 HTTPS 地址。
...
- ntfy 可用于 Android、Wear OS、普通手机通知，以及 Quick Tunnel 新链接提醒脚本。
...
| 没收到新的 ntfy 链接 | 确认手机订阅了 `NTFY_NOTIFY_TOPIC`，检查运行目录中的 `notify-error.log`，并确认当前链接与上次不同 |
