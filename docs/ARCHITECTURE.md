...
flowchart TB
    ...
    subgraph Desktop[桌面电脑]
        ...
        LaunchAgent[macOS LaunchAgent<br/>登录启动与异常恢复]
        Auth[令牌认证]
...
长期使用推荐 Tailscale Serve：

- PocketCodex 仍只监听 loopback，Tailscale 在 tailnet 内提供固定 HTTPS 主机名。
- 手机和电脑需要加入同一 tailnet；设备身份和 ACL 构成令牌以外的第二层访问控制。
- `setup_tailscale_serve.py` 读取 Tailscale 结构化状态，配置 Serve，并验证固定地址的首页和 sessions API。
- macOS 用户级 LaunchAgent 只负责维持 `remote_codex_server.py`；Tailscale 自己负责 Serve 配置和网络重连。
- 服务进程、Tailscale 连接和固定主机名相互解耦。电脑休眠或切换网络后，原主机名保持不变，恢复连接后手机刷新原页面即可。
- LaunchAgent plist 只包含 Python、仓库和日志路径，不包含 token、ntfy topic 或完整访问链接。

### Cloudflare Quick Tunnel

面向临时测试和无 Tailscale 环境的备用路径：
...
- URL 重启后通常改变，不适合作为固定服务地址，也不能通过页面定时刷新变成稳定入口。
- 当前方案没有 Cloudflare Access 身份策略，不应视为私有网络。

### 多电脑使用

PocketCodex 当前没有中心设备注册服务。一个手机连接多台电脑时，推荐把电脑加入同一 tailnet，每台电脑运行独立服务并使用自己的固定设备主机名和 token，手机通过不同书签或主屏幕图标切换。多电脑统一入口仍需要额外的设备身份、撤销和在线状态设计，不能共享一个 token。
...
1. 保持服务绑定 loopback。
2. 长期访问优先使用 Tailscale Serve，并把 tailnet 成员控制在必要范围内。
3. 把 `remote.env` 当作密码文件处理。
4. 只开放必要项目根目录。
5. 令牌疑似泄漏时立即轮换。
6. 使用 Quick Tunnel 临时入口时，把完整 URL 当作密码，并接受地址可能随时失效。
