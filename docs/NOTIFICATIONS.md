...
## Tailscale Serve 固定入口通知

macOS 运行 `python3 setup_tailscale_serve.py` 后，脚本会验证固定 tailnet 地址的首页和带令牌 sessions API。配置了 `NTFY_NOTIFY_TOPIC` 时，它会把固定入口发送到手机一次。

Tailscale Serve 的设备主机名不会因为电脑休眠、网络切换或连接器重连而轮换，因此：

- 手机应把该地址加入书签或主屏幕；
- 相同固定地址不会重复推送；
- 临时断线时等待电脑恢复，然后刷新原页面；
- ntfy 继续负责任务完成、审批和异常提醒，而不是日常入口发现。

只有设备主机名或 PocketCodex token 改变时，才需要重新发送并打开固定链接。手机和电脑必须登录同一个 Tailscale tailnet。

## Quick Tunnel 新链接通知（临时入口）

跨平台辅助脚本 `start_remote_codex.py` 可以复用同一个 `NTFY_NOTIFY_TOPIC`，在 Cloudflare Quick Tunnel 地址变化后把新的 PocketCodex 链接发送到手机。Quick Tunnel 是随机临时入口，该通知流程不能提供固定书签体验。
...
- `setup_tailscale_serve.py` 在 macOS 配置固定 Tailscale Serve 地址、持久 LaunchAgent，并可发送一次固定入口通知。
